...
name += "__mu_" + tfobs.run_utils.float2str(args.mu)
if args.lr is not None:
  name += "__lr_" + tfobs.run_utils.float2str(args.lr)
else:
  name += "__lr_sched_" + tfobs.run_utils.float2str(args.lr_sched_vals[0])
  for step, val in zip(args.lr_sched_steps, args.lr_sched_vals[1:]):
    name += "_" + str(step) + "_" + tfobs.run_utils.float2str(val)
...
# Set up optimizer
opt = tf.train.MomentumOptimizer(learning_rate, momentum=args.mu,
                                 use_nesterov=False)
step = opt.minimize(loss, var_list=variables, global_step=global_step)
